    ...
    requirejs.tools.useLib(function (require) {
      rjsConfig = require('transform').modifyConfig(config, function (config) {
        ...
        if (generatedConfig.packages) {
          ...
          _.each(generatedConfig.packages, function(generatedPackage) {
            var existingPackage = _.find(config.packages, function(findPackage) { return findPackage.name == generatedPackage.name; });
            if(existingPackage) {
              existingPackage.location = generatedPackage.location;
              existingPackage.main = generatedPackage.main;
            } else {
              config.packages.push(generatedPackage);
            }
          });
        }
